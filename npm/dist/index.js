...
exports.HierarchicalSplineLinear = exports.LowRankLinear = exports.SplineLinear = exports.EisensteinLattice = exports.VARIANT_GUIDE = exports.GAUSSIAN_BASIS = exports.BSPLINE_BASIS = exports.EISENSTEIN_BASIS = void 0;
exports.compressionRatio = compressionRatio;
exports.recommendVariant = recommendVariant;
exports.serializeSplineLinear = serializeSplineLinear;
exports.deserializeSplineLinear = deserializeSplineLinear;
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
/** Basis constant: Eisenstein (inverse-distance-squared on hex lattice) */
exports.EISENSTEIN_BASIS = 'eisenstein';
/** Basis constant: Bicubic B-spline on regular grid */
exports.BSPLINE_BASIS = 'bspline';
/** Basis constant: Gaussian RBF on Eisenstein lattice */
exports.GAUSSIAN_BASIS = 'gaussian';
const SQRT3_HALF = Math.sqrt(3) / 2;
const VALID_BASES = new Set(['eisenstein', 'bspline', 'gaussian']);
exports.VARIANT_GUIDE = {
    smooth: {
        name: 'spline',
        method: 'spline',
...
class EisensteinLattice {
    constructor(nPoints) {
        if (nPoints < 1)
            throw new Error(`nPoints must be >= 1, got ${nPoints}`);
    ...
    nearestK(point, k = 4) {
        ...
        const dists = this._positions.map(([px, py]) => Math.sqrt((px - point[0]) ** 2 + (py - point[1]) ** 2));
...
exports.EisensteinLattice = EisensteinLattice;
// ---------------------------------------------------------------------------
// SplineLinear
// ---------------------------------------------------------------------------
/**
 ...
 */
class SplineLinear {
    constructor(inFeatures, outFeatures, nControlPoints = 16, basis = 'eisenstein', bias = true) {
        ...
        this.controlValues = Array.from({ length: nControlPoints }, () => (Math.random() * 2 - 1) * bound);
    ...
    toString() {
        return `SplineLinear(${this.inFeatures}→${this.outFeatures}, knots=${this.nControlPoints}, params=${this.numTrainableParams()})`;
    }
    // --- Basis implementations ---
    _basisEisenstein(wFlat) {
...
exports.SplineLinear = SplineLinear;
// ---------------------------------------------------------------------------
// LowRankLinear
// ---------------------------------------------------------------------------
/**
 ...
 */
class LowRankLinear {
    constructor(inFeatures, outFeatures, rank = 16, bias = true, initScale = 0.01) {
        this.inFeatures = inFeatures;
        this.outFeatures = outFeatures;
        this.rank = rank;
    ...
    forward(x) {
        if (Array.isArray(x[0])) {
            return x.map(sample => this._apply(sample));
        }
        return this._apply(x);
    }
    _apply(x) {
        // x @ U → (rank,)
...
exports.LowRankLinear = LowRankLinear;
// ---------------------------------------------------------------------------
 ...
 */
class HierarchicalSplineLinear {
    constructor(inFeatures, outFeatures, coarsePts = 16, finePts = 8, patchSize = 32, alphaInit = 0.5, bias = true) {
        this.inFeatures = inFeatures;
        this.outFeatures = outFeatures;
        ...
        this.fineValues = Array.from({ length: this.totalPatches }, () => Array.from({ length: finePts }, () => randn() * 0.01));
    ...
    materializeWeights() {
        ...
        for (let i = 0; i < this.outFeatures; i++) {
            ...
            for (let j = 0; j < this.inFeatures; j++) {
                ...
                coarseW[i * this.inFeatures + j] = idwInterpolate([x, y], coarsePositions, this.coarseValues);
            }
        }
            ...
            for (let pj = 0; pj < this.nOutPatches; pj++) {
                ...
                for (let i = 0; i < patchRows; i++) {
                    for (let j = 0; j < patchCols; j++) {
                        ...
                        const fineW = idwInterpolate([lx, ly], finePositions, this.fineValues[patchIdx]);
...
exports.HierarchicalSplineLinear = HierarchicalSplineLinear;
// ---------------------------------------------------------------------------
// Utility functions
// ---------------------------------------------------------------------------
/**
 ...
 */
function compressionRatio(inFeatures, outFeatures, controlPoints, basis = 'eisenstein', bias = true) {
...
function serializeSplineLinear(layer) {
    return {
        type: 'spline_linear',
...
function deserializeSplineLinear(data) {
    const layer = new SplineLinear(data.inFeatures, data.outFeatures, data.nControlPoints, data.basis, data.bias !== null);
    layer.controlValues = data.controlValues;
    if (layer.basis === 'gaussian')
        layer.logBandwidth = data.logBandwidth;
    if (data.bias)
...
// Internal helpers
// ---------------------------------------------------------------------------
function idwInterpolate(point, positions, values) {
    let sumKernel = 0;
    let sumWeighted = 0;
...
function bSplineBasis(d) {
    if (d < 1)
        return (2 / 3) + d * d * (d / 2 - 1);
    if (d < 2)
        return (1 / 6) * (2 - d) * (2 - d) * (2 - d);
    return 0;
}
